# --------------------------------------------------------------------
# RUNNING LOOKOUT ON DIFFERENT MODELS - BU APPROACH ***********
...
fore_res_ets <- fore_res %>%
  filter(.model == 'ets_bu', nodes != '<aggregated>')
head(fore_res_ets)
...
saveRDS(out_all, file = "Data_Output/To_Make_Public/All_outliers_1_week_moving_window_alpha_point1.rds")
write_csv(anomaly_stats, "Data_Output/To_Make_Public/Anomaly_stats_1_week_moving_window.csv")
...
fore_res_tslm <- fore_res %>%
  filter(.model == 'tslm_bu', nodes != '<aggregated>')
head(fore_res_tslm)
...
colnames(anomaly_stats) <- c("Number_of_ARP_Calls", "Number_Unique_Nodes", "Number_Anomaly_Calls", "Number_Unique_Anomalies")
saveRDS(out_all, file = "Data_Output/To_Make_Public/TSLM_All_outliers_1_week_moving_window_alpha_point1.rds")
write_csv(anomaly_stats, "Data_Output/To_Make_Public/TSLM_Anomaly_stats_1_week_moving_window.csv")
...
fore_res_lgbm <- fore_res %>%
  filter(hf_model == 'BU',fore_model =="Lightgbm", node != 0)  # '<aggregated>' = 0 for this set
head(fore_res_lgbm)
...
colnames(anomaly_stats) <- c("Number_of_ARP_Calls", "Number_Unique_Nodes", "Number_Anomaly_Calls", "Number_Unique_Anomalies")
saveRDS(out_all, file = "Data_Output/To_Make_Public/Lightgbm_All_outliers_1_week_moving_window_alpha_point1.rds")
write_csv(anomaly_stats, "Data_Output/To_Make_Public/Lightgbm_Anomaly_stats_1_week_moving_window.csv")
...
fore_res_zero <- fore_res %>%
  filter(hf_model == 'BU',fore_model =="Zero-Inflated", node != 0)  # '<aggregated>'
head(fore_res_zero)
table(fore_res_zero$fore_model)
...
colnames(anomaly_stats) <- c("Number_of_ARP_Calls", "Number_Unique_Nodes", "Number_Anomaly_Calls", "Number_Unique_Anomalies")
saveRDS(out_all, file = "Data_Output/To_Make_Public/Zeroinflated_All_outliers_1_week_moving_window_alpha_point1.rds")
write_csv(anomaly_stats, "Data_Output/To_Make_Public/Zeroinflated_Anomaly_stats_1_week_moving_window.csv")
