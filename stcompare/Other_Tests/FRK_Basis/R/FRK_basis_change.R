# Goal: Run FRK with various sized bases, controlled by
# the regular and nres parameters
...
#------------------------------------------------------------------------
set.seed(1)
# Load the grid of prediction points
load("Data_Inputs/Africa_grids_mh.RData")
grid <- grid_cut_mh

# Load the polygons of the Africa map, to use for masking the outpyt
Africa_map <- readRDS("Data_Inputs/Africa_border.rds")
...
# change into an sp object with coordinates x and y
coordinates(zdf) = ~x+y
...
regular <- 1                # Control's number of functions in each direction
scale_aperture <- 1.25      # Control's spatial scale of the basis functions
...
# The training data and BAUs require the same coordinate reference system
# which should be the WGS84 CRS
...
result <- list(data = FRK_prediction_data,
               time = FRK_times,
...
result$metadata$nres <- nres
result$metadata$regular <- regular
result$metadata$scale <- scale_aperture

# Put the metadata into the filename
