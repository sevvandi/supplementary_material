...
# In this file we use the Vecchia approximation with other parameters as default,
# to see how fast it runs and what the plots look like.
...
# Set the working directory to the "GPBoost_Vecchia" directory
setwd()
...
gp_model <- GPModel(gp_coords = coords,
                    ...
                    vecchia_approx = TRUE, # Use vecchia approx
                    num_neighbors = mv
                    )
...
pred <- predict(bst, data = X_test, gp_coords_pred = grid@coords,
                ...
                num_neighbors_pred = mvp) # predict_var turned to false due to memory requirements using vecchia
...
result$metadata$vecc <- "VECCHIA"
result$metadata$mv <- mv
result$metadata$mvp <- mvp

metadata_tail <- ""
for (i in 1:length(result$metadata)){
  metadata_tail <- paste0(metadata_tail, names(result$metadata[i]), result$metadata[[i]])
}



output_file <- "Data_Outputs/"
output_file <- paste0("Data_Outputs/", metadata_tail, ".rds")
