...
res <- inla(formula, family = "gaussian",
            # control.family = list(link = "logit"), no link function with Gaussian response
            data = inla.stack.data(stk.full),
            control.predictor = list(compute = TRUE,
                                     # link = 1, no link function with Gaussian response
...
result$metadata$response <- "gaussian"
