...
# Set the working directory to the Africa folder
setwd()
...
# Mean of response
predresponse <- predict(mod, test, type = "response", num.threads = 1)$predictions
...
pred_grid_masked$iqr <- pred_grid_masked$q0.75 - pred_grid_masked$q0.25
pred_grid_masked$sd <- pred_grid_masked$iqr/1.34898

# Save prediction data
#------------------------------------------------------------------------
SpRF_prediction_data  <- as.data.frame(pred_grid_masked)
# Use the median for prediction
...
result <- list(data = SpRF_prediction_data,
               times = SpRF_times,
