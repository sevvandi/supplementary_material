...
# Set the working directory to the Africa_Prediction_Maps directory
setwd()
...
# Need to convert max.edge and cutoff to radians (otherwise the mesh will be coarse)
...
train_coord_cartesian <- inla.mesh.map(train_coord_longlat,
                                       projection = "longlat",
                                       inverse = TRUE)

predict_coords_longlat <- grid@coords
predict_coord_cartesian <- inla.mesh.map(predict_coords_longlat,
                                         projection = "longlat",
                                         inverse = TRUE)
...
# Compute the mean. Note we are using median instead for prediction
...
# SD of the predive distribution
...
INLA_prediction_data <- cbind.data.frame(predict_coords_longlat,
                                         pred,
                                         pred_confs,
                                         mean,
                                         pred_SD)
...
result <- list(data = INLA_prediction_data,
               time = INLA_times,
